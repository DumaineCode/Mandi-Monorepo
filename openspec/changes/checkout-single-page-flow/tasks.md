...
- [x] **2c.3** No Stripe branch in the new section. Per `design.md` §0 CONFLICT-1 RESOLUTION, `apps/backend/medusa-config.ts` registers only `openpay` and `mercadopago`; **R5 applies universally with no provider carve-out.**
...
- [x] **2c.32** Apply `size:exception` with the `design.md` §10 floor justification, and note that the Stripe deletion reduced this PR from ~1 000 to ~900 but did **not** bring it under the 600 budget.
...
- [ ] **2c.40 — MAJOR / MANUAL QA (blocking merge).** The sticky bar covered the summary, the discount field and the trust badges on mobile. The reservation was `pb-[calc(6rem+env(safe-area-inset-bottom))]` on the FORM COLUMN only (`apps/storefront/src/modules/checkout/templates/checkout-form/index.tsx:92`), while the page is `grid-cols-1` with no `gap-y` (`apps/storefront/src/app/[countryCode]/(checkout)/checkout/page.tsx:53`), so `CheckoutSummary` and the layout footer rendered *below* the reserved padding. Measured bar height on a notched iPhone with the cart resolved: 12 + 36 + 48 + 8–26 + 12 + safe-area ≈ **150–168 px** against a **~130 px** reservation that did not even apply to the elements at risk. Fixed by moving the reservation to the page grid at `10rem` + safe-area, and adding matching clearance to the checkout layout footer (`(checkout)/layout.tsx:88`) since it is a sibling of `{children}` and therefore below any page-level padding. **Verify on a real notched phone (not a devtools emulator — `env(safe-area-inset-bottom)` is `0px` there):**
  1. Scroll the checkout to **maximum scroll**.
  2. The **discount code field** MUST be fully visible and tappable, not under the bar. Type a code and submit it.
  3. The **payment badges** in the footer MUST be fully visible.
  4. The last form field and the legal text MUST clear the bar.
  5. Repeat with the CTA disabled and a **two-line** missing message in the bar — the bar is at its tallest there.
  6. Check `payment/mercadopago/status` still looks right on mobile; it pays for the footer clearance with whitespace it should never scroll to.
- [ ] **2c.41 — MAJOR / MANUAL QA (blocking merge).** A `provider-config` failure left Openpay as an infinite skeleton. `getProviderConfig()` returns `{ openpay: null }` on any failure, `PaymentWrapper` then renders `<div>{children}</div>` with **no `OpenpayContext.Provider`** (`payment-wrapper/index.tsx:136`), and `PaymentSection` still renders the Openpay row because it reads `availablePaymentMethods` — so `OpenpayCardContainer` read the **default** context, which had `unavailable: false`, and rendered `<SkeletonCardDetails />` forever with no timeout. Fixed by defaulting `unavailable: true` (`payment-wrapper/openpay-wrapper.tsx` — `OPENPAY_CONTEXT_DEFAULT`), covered by `openpay-wrapper.spec.ts` (5 cases, 4 mutants killed). **Verify with the endpoint actually down:** block `GET /store/provider-config` in devtools, reload checkout, select *Tarjeta de crédito/débito*. The `openpay-unavailable-message` MUST render. There MUST be no skeleton.
- [ ] **2c.42 — MINOR, recorded not fixed.** Under 2c.41's failure the Openpay row still renders and is still selectable, so a customer who picks it reads "Card payments are temporarily unavailable…" in the section while the CTA reports `Completa los datos de tu tarjeta.` — two vocabularies for one condition, which is this change's own named defect class. Closing it means either filtering the Openpay row out of `availablePaymentMethods` when `openpayConfig` is `null` (threads the config through `CheckoutForm` into `PaymentSection`) or giving `card_details` an unavailable-aware variant. Not done here: the customer-blocking half — the infinite skeleton with no message at all — is closed, and the remaining defect is wording under a backend outage. `apps/storefront/src/lib/util/checkout-readiness.ts:664` and `apps/storefront/src/modules/checkout/components/payment-section/index.tsx:131`.
- [ ] **2c.43 — MAJOR / MANUAL QA (blocking merge). DECISION: RATIFIED (maintainer, pre-merge).** A failed 3DS or Mercado Pago payment returned the customer to a checkout that said nothing. `payment/openpay/return/route.ts:29-32` and `payment/mercadopago/failure/route.ts:22-24` both redirect to `?error=payment_failed`, which had **zero** readers under `(checkout)/`. The customer read the pristine checkout as "my click didn't register" and retried — a **second authorization hold** on the same card. Fixed by `selectCheckoutEntryError` (`apps/storefront/src/lib/util/place-order.ts`, 5 spec cases, 4 mutants killed) seeded into `state.error` through `CheckoutInit` (`checkout/page.tsx:74,119`). The `error=payment_failed` non-goal in `proposal.md`, `design.md` §5/§13 and this file's Non-goals section is **retired** — both documents amended to match. The box stays open only for the device verification below; the product decision is closed. **Verify:**
  1. Openpay: force a 3DS decline. Confirm the return lands on `/mx/checkout?error=payment_failed` and the message renders beside the CTA.
  2. Mercado Pago: abandon MP's checkout. Same expectation.
  3. Confirm the message is **noticed** without hunting — the customer lands at the top of the page and the message renders at the bottom beside the CTA. If QA finds it is missed, promote it to a dismissible banner above `ContactAddressSection`; the pure rule does not change.
  4. Click *Realizar pedido* again. The message MUST clear when the attempt starts.
  5. Hand-craft `/mx/checkout?error=<arbitrary text>`. **Nothing** may render — the rule matches the literal `payment_failed` only, because the query string is attacker-controlled and this is the moment the customer is typing a PAN.
- [x] **2c.44 — MINORs, fixed.** (a) `apps/storefront/src/modules/checkout/components/payment-button/index.tsx:15` — `PLACE_ORDER_LABEL` was exported with no consumer, defended by a docstring about a drift that cannot happen (both variants render the same button, which renders the constant). De-exported; the claim is corrected rather than the code contorted to match it. (b) `apps/storefront/src/modules/checkout/components/billing_address/index.tsx:118-125` — `province` is in `REQUIRED_ADDRESS_FIELDS` and blocks the CTA, but was not marked `required`; A6's own motivation names `city` **and** `province` and only `city` got it. (c) `apps/storefront/src/lib/util/place-order.ts` — `PLACE_ORDER_MESSAGES.deviceSessionMissing` said "Recarga la página", which was correct for the PR1b mount trigger and is wrong under §12b: a reload loads nothing until the customer re-selects Openpay, so following the instruction changes nothing. Now says to wait a moment. (d) `apps/storefront/src/lib/constants.tsx` — the Mercado Pago row gained the off-site caption.
- [ ] **2c.45 — MINOR, recorded not fixed (needs a spec decision).** `apps/storefront/src/lib/util/checkout-readiness.ts:688` — `canPlaceOrder` has no non-spec callers; `place-order-flow.ts:291` and the new step 3.5 both call `getMissingOrderRequirements` directly. **Not deleted**, because `design.md` §2 Module B lists it as part of this module's mandated public API and per RC-1 the spec owns the WHAT. Either amend `design.md` §2 to drop it, or leave it as the published predicate it is documented to be. It is exercised by 8 assertions in `checkout-readiness.spec.ts`, so it is not untested — only uncalled.
- [ ] **2c.46 — MINOR, recorded not fixed (split decision).** The gift-card bypass survives in `apps/storefront/src/lib/util/checkout-readiness.ts:652` (`if (!input.paidByGiftCard)`), but the "Gift card" summary UI it depended on was deleted with `review/index.tsx` (2c.20). A gift-card cart would therefore show **no payment UI and an enabled CTA**. It is documented unreachable in this deployment — `gift_cards` is not on Medusa v2's `StoreCart`, so `readPaidByGiftCard` can only ever answer `false` — and `design.md` D2 lists the input field, which is why the branch was kept. **The split is deliberate and recorded here rather than silently resolved either way:** enabling gift cards is a change that must re-open this branch AND restore a payment surface for it, not one that can assume either exists.
- [x] **2c.39 — SPEC/DESIGN CONFLICT, RESOLVED (maintainer, pre-merge). Single button wins.** `specs/storefront-checkout/spec.md` scenario *The last form field is reachable* required "the legal text **and the in-flow CTA** are visible above the sticky bar, not underneath it". `design.md` D9 and task 2c.17 required the opposite — the in-flow CTA is desktop-only (`hidden small:block`) and the bar MUST NOT render on desktop, so the two CTAs never coexist. As built, the in-flow CTA is not visible on mobile at any scroll position, which made the scenario literally unsatisfiable. **Decision: S2 / 2c.24 ("exactly one order-placement button") wins — no changes to the implementation.** The spec scenario is amended: legal text and the itemized list stay visible in flow; the in-flow CTA stays desktop-only and does not duplicate onto mobile.
...
- [ ] **T.5** Open the recorded follow-ups, none of them in scope here: `retrieveCart`'s own `force-cache`-with-possibly-empty-tag issue (`design.md` §13); `isStripeLike` removal once the order module drops it (RC-4); variant dimension data quality (`proposal.md` §4). (`error=payment_failed` is no longer a follow-up — retired and shipped in 2c.43.)
