...
- [ ] **2c.43 — MAJOR / MANUAL QA (blocking merge) — and a deliberate NON-GOAL override.** A failed 3DS or Mercado Pago payment returned the customer to a checkout that said nothing. `payment/openpay/return/route.ts:29-32` and `payment/mercadopago/failure/route.ts:22-24` both redirect to `?error=payment_failed`, which had **zero** readers under `(checkout)/`. The customer read the pristine checkout as "my click didn't register" and retried — a **second authorization hold** on the same card. Fixed by `selectCheckoutEntryError` (`apps/storefront/src/lib/util/place-order.ts`, 5 spec cases, 4 mutants killed) seeded into `state.error` through `CheckoutInit` (`checkout/page.tsx:74,119`). **Making `error=payment_failed` visible is recorded as a non-goal in `proposal.md` §4, `design.md` §13 and this file's Non-goals section; it is overridden here on purpose** — that non-goal was written when `?step=` deadlocked the page, so the customer could not have acted on the message. Remove it from the Non-goals list, or reject this fix; do not leave both standing. **Verify:**
